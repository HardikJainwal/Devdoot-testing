...
export default function AllDoctorsPage() {
  ...
  const filteredDoctors = doctors.filter(doctor => {
    ...
    const matchesPrice = doctor.fees >= filters.priceRange[0] && 
                        doctor.fees <= filters.priceRange[1];
    
    const matchesExperience = doctor.experience >= filters.experienceRange[0] && 
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className={`${poppins.className} text-lg text-red-600 mb-4`}>{error}</p>
          <button
          ...
          <div className={`${showFilters ? 'block' : 'hidden'} lg:block w-full lg:w-80 bg-white rounded-lg shadow-sm p-6 h-fit`}>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                onChange={(e) => handleFilterChange('experienceRange', [parseInt(e.target.value), filters.experienceRange[1]])}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              />
            </div>
              ...
              <div className="flex items-center space-x-2">
                {[1, 2, 3, 4, 5].map(star => (
                  <button
                    ...
                    className={`${star <= filters.rating ? 'text-yellow-400' : 'text-gray-300'}`}
                  >
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h3 className={`${poppins.className} text-xl font-semibold text-gray-900 mb-1`}>
                                {doctor.name || 'Dr. Name'}
                              </h3>
                              <p className={`${poppins.className} text-sm text-gray-600 mb-1`}>
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                            ...
                            {doctor.consultationModes?.includes('video') && (
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${poppins.className} bg-green-100 text-green-700`}>
                      ...
                      <div className="sm:w-48 flex flex-col justify-between">
                        <div className="text-right mb-4">
                          <div className="text-right mb-2">
                            <span className={`${poppins.className} text-2xl font-bold text-gray-900`}>
                              ₹{doctor.fees || '1200'}
                            </span>
                            <span className={`${poppins.className} text-sm text-gray-600 block`}>
                              per session
                            </span>
